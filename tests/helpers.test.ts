...
describe('Helper Functions', () => {
  describe('getTokensByTags', () => {
    beforeEach(async () => {
      // Create test tokens
      await Token.create([
        { token: 'token1', tags: ['sports', 'news'], locale: 'en', platform: 'ios' },
    ...
    it('should return tokens with matching tags', async () => {
      ...
      expect(tokens.map(t => t.token)).toEqual(expect.arrayContaining(['token1', 'token2']));
    });

    it('should return tokens with any of the specified tags', async () => {
      const tokens = await getTokensByTags(['sports', 'premium']);
      expect(tokens).toHaveLength(3);
      expect(tokens.map(t => t.token)).toEqual(expect.arrayContaining(['token1', 'token2', 'token4']));
    });
    ...
    it('should throw error for invalid input', async () => {
      await expect(getTokensByTags([])).rejects.toThrow('Tags must be a non-empty array');
      await expect(getTokensByTags(null as any)).rejects.toThrow('Tags must be a non-empty array');
    });
  });
  ...
  describe('groupTokensByLocale', () => {
    ...
    it('should throw error for invalid input', () => {
      expect(() => groupTokensByLocale(null as any)).toThrow('Tokens must be an array');
      expect(() => groupTokensByLocale('invalid' as any)).toThrow('Tokens must be an array');
    });
  });
  ...
  describe('generatePayload', () => {
    ...
    it('should throw error for invalid input', () => {
      expect(() => generatePayload(null as any)).toThrow('Locales content must be an object');
      expect(() => generatePayload('invalid' as any)).toThrow('Locales content must be an object');
    });
    ...
    it('should validate content structure', () => {
      ...
      expect(() => generatePayload(invalidContent)).toThrow('Invalid content for locale en');
    });
  });
  ...
  describe('removeInvalidTokens', () => {
    ...
    it('should remove specified tokens', async () => {
      ...
      expect(remainingTokens.map(t => t.token)).toEqual(expect.arrayContaining(['valid1', 'valid2']));
    });
    ...
    it('should filter out invalid token entries', async () => {
      ...
      await removeInvalidTokens(['invalid1', '', null as any, 'invalid2', undefined as any]);
