...
import { getScenario, listScenarios, validateScenario } from './stress-test-configs.js';
...
program
  ...
  .action(async (options: QueueTestOptions) => {
    try {
      ...
      const theoreticalMaxThroughput = (config.batchSize * config.concurrency) / (50 / 1000);
      const efficiency = (results.throughput / theoreticalMaxThroughput) * 100;

      console.log(`\n📈 Efficiency: ${efficiency.toFixed(2)}%`);
      console.log(`Theoretical Max: ${theoreticalMaxThroughput.toFixed(2)} jobs/s`);
...
program
  ...
  .action(async (options: FullTestOptions) => {
    try {
      ...
      console.log('\n📊 API Test Results:');
      console.log(`Total Requests: ${apiResults.totalRequests.toLocaleString()}`);
      console.log(`Successful: ${apiResults.successfulRequests.toLocaleString()}`);
      ...
      console.log(`✅ ${options.size} notifications processed in: ${(queueResults.processingTime / 1000).toFixed(2)}s`);
...
program
  ...
  .action(async (name) => {
    try {
      ...
      console.log(`✅ API can handle: ${apiResults.throughput.toFixed(2)} req/s`);
      console.log(`✅ Queue can process: ${queueResults.throughput.toFixed(2)} notifications/s`);
      console.log(`✅ ${config.queueSize.toLocaleString()} notifications processed in: ${(queueResults.processingTime / 1000).toFixed(2)}s`);
      console.log(`✅ P95 API response time: ${apiResults.p95ResponseTime.toFixed(2)}ms`);
      console.log(`✅ P99 API response time: ${apiResults.p99ResponseTime.toFixed(2)}ms`);
...
program
  .command('quick')
  .description('Quick test with default settings (30s API test, 10k queue test)')
  .action(async () => {
    try {
      ...
      console.log(`✅ API throughput: ${apiResults.throughput.toFixed(2)} req/s`);
      console.log(`✅ P95 response time: ${apiResults.p95ResponseTime.toFixed(2)}ms`);
      ...
      console.log(`✅ Queue throughput: ${queueResults.throughput.toFixed(2)} notifications/s`);
      console.log(`✅ 10k notifications processed in: ${(queueResults.processingTime / 1000).toFixed(2)}s`);
