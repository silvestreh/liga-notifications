...
const mockSendAPNsBatch = async (tokens: string[], payload: any) => {
  ...
  const invalidCount = Math.floor(tokens.length * (Math.random() * 0.04 + 0.01));
...
class StressTester {
  ...
  private errors: Map<string, { status: number; message: string; count: number }> = new Map();
  ...
  async testAPIEndpoints(): Promise<TestResult> {
    console.log(`🚀 Starting API stress test for ${this.config.testDuration}s at ${this.config.requestsPerSecond} req/s`);
    ...
    while (performance.now() < endTime) {
      ...
      const targetDelay = (1000 / this.config.requestsPerSecond) * this.config.concurrency;
  ...
  private calculateResults(): void {
    ...
    if (this.responseTimes.length > 0) {
      ...
      this.results.p95ResponseTime = this.responseTimes[Math.floor(this.responseTimes.length * 0.95)];
      this.results.p99ResponseTime = this.responseTimes[Math.floor(this.responseTimes.length * 0.99)];
    }

    this.results.throughput = this.results.totalRequests / this.config.testDuration;
...
class QueueStressTester {
  ...
  private async initialize() {
    ...
    console.log(`📊 Starting queue stress test with ${this.config.queueSize} notifications`);
    ...
    for (let i = 0; i < this.config.queueSize; i += 100) {
      ...
      const tokens = Array.from({ length: batchSize }, (_, index) =>
        `test_token_${i + index}_${Date.now()}`
      );
...
async function runStressTests() {
  try {
    ...
    console.log(`Success Rate: ${((apiResults.successfulRequests / apiResults.totalRequests) * 100).toFixed(2)}%`);
    console.log(`Throughput: ${apiResults.throughput.toFixed(2)} req/s`);
    console.log(`Average Response Time: ${apiResults.averageResponseTime.toFixed(2)}ms`);
    console.log(`P95 Response Time: ${apiResults.p95ResponseTime.toFixed(2)}ms`);
    console.log(`P99 Response Time: ${apiResults.p99ResponseTime.toFixed(2)}ms`);
    console.log(`Min Response Time: ${apiResults.minResponseTime.toFixed(2)}ms`);
    console.log(`Max Response Time: ${apiResults.maxResponseTime.toFixed(2)}ms`);
    ...
    console.log(`Processing Time: ${(queueResults.processingTime / 1000).toFixed(2)}s`);
    console.log(`Average Job Time: ${queueResults.averageJobTime.toFixed(2)}ms`);
    ...
    const theoreticalMaxThroughput = (config.batchSize * config.concurrency) / (50 / 1000); // Assuming 50ms per batch
    const efficiency = (queueResults.throughput / theoreticalMaxThroughput) * 100;

    console.log(`\n📈 Efficiency Analysis:`);
    console.log(`Theoretical Max Throughput: ${theoreticalMaxThroughput.toFixed(2)} jobs/s`);
    console.log(`Actual Throughput: ${queueResults.throughput.toFixed(2)} jobs/s`);
    ...
    console.log(`✅ Queue can process: ${queueResults.throughput.toFixed(2)} notifications/s`);
    console.log(`✅ 100k notifications processed in: ${(queueResults.processingTime / 1000).toFixed(2)}s`);
    console.log(`✅ P95 API response time: ${apiResults.p95ResponseTime.toFixed(2)}ms`);
    console.log(`✅ P99 API response time: ${apiResults.p99ResponseTime.toFixed(2)}ms`);
