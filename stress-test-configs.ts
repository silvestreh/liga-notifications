...
export const stressTestScenarios: StressTestScenario[] = [
  ...
  },
  {
    name: "heavy",
    description:
      "Heavy load testing - suitable for production capacity planning",
    config: {
      apiUrl: "http://localhost:3000",
      apiKey: "test-api-key",
      testDuration: 120,
  ...
  },
  {
    name: "queue-focused",
    description:
      "Queue processing focused test - minimal API load, maximum queue processing",
    config: {
      apiUrl: "http://localhost:3000",
      apiKey: "test-api-key",
      testDuration: 10,
  ...
  },
  {
    name: "api-focused",
    description:
      "API endpoint focused test - maximum API load, minimal queue processing",
    config: {
      apiUrl: "http://localhost:3000",
      apiKey: "test-api-key",
      testDuration: 180,
...
export function listScenarios(): void {
  ...
  stressTestScenarios.forEach((scenario) => {
    ...
    console.log(
      `   API: ${scenario.config.testDuration}s @ ${scenario.config.requestsPerSecond} req/s`,
    );
    console.log(
      `   Queue: ${scenario.config.queueSize.toLocaleString()} notifications @ ${scenario.config.concurrency} workers`,
    );
...
export function validateScenario(config: StressTestConfig): string[] {
  ...
  if (config.requestsPerSecond > 10000) {
    errors.push(
      "Requests per second should not exceed 10,000 (adjust concurrency instead)",
    );
  }

  if (config.concurrency > 100) {
    errors.push(
      "Concurrency should not exceed 100 (may cause resource exhaustion)",
    );
  }
